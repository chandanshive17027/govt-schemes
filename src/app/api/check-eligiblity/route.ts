// // src/app/api/check-eligibility/route.ts
// import { NextRequest, NextResponse } from "next/server";
// import { PrismaClient } from "@prisma/client";
// import { spawn } from "child_process";
// import fs from "fs";
// import path from "path";
// import { sendEmail } from "@/utils/actions/notifications/notifications";

// const prisma = new PrismaClient();

// // Helper to normalize state and check match
// function isStateMatch(userState: string | undefined, schemeStates: string[] | string | undefined, schemeMinistry?: string) {
//   if (!userState) return false;
//   const userStateNorm = userState.trim().toLowerCase();

//   const statesArr: string[] = Array.isArray(schemeStates)
//     ? schemeStates
//     : schemeStates
//     ? [schemeStates]
//     : [];

//   if (statesArr.length === 0 || statesArr.map(s => s.trim().toLowerCase()).includes("all")) {
//     return true;
//   }

//   for (const s of statesArr) {
//     const sNorm = s.trim().toLowerCase();
//     if (schemeMinistry && sNorm.startsWith(schemeMinistry.trim().toLowerCase())) {
//       return true;
//     }
//     if (sNorm === userStateNorm || userStateNorm.includes(sNorm) || sNorm.includes(userStateNorm)) {
//       return true;
//     }
//   }
//   return false;
// }

// // Helper to run Python eligibility script
// async function runPython(user: any, schemes: any[]) {
//   return new Promise<any[]>((resolve, reject) => {
//     try {
//       const tempDir = path.join(process.cwd(), "tmp");
//       if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir);

//       const userFile = path.join(tempDir, `user_${Date.now()}.json`);
//       const schemesFile = path.join(tempDir, `schemes_${Date.now()}.json`);

//       // Pre-filter schemes based on state match like notifications
//       const filteredSchemes = schemes.filter(scheme =>
//         isStateMatch(user.state, scheme.state, scheme.ministry)
//       );

//       fs.writeFileSync(userFile, JSON.stringify(user));
//       fs.writeFileSync(schemesFile, JSON.stringify(filteredSchemes));

//       const py = spawn("python3", [
//         path.join(process.cwd(), "src/scripts/user_eligibility.py"),
//         userFile,
//         schemesFile,
//       ]);

//       let output = "";
//       let error = "";

//       py.stdout.on("data", (data) => {
//         output += data.toString();
//       });

//       py.stderr.on("data", (data) => {
//         error += data.toString();
//       });

//       py.on("close", (code) => {
//         fs.unlinkSync(userFile);
//         fs.unlinkSync(schemesFile);

//         if (code === 0) {
//           try {
//             const parsed = JSON.parse(output);
//             resolve(parsed);
//           } catch (e) {
//             reject("Invalid JSON from Python: " + output);
//           }
//         } else {
//           reject(error || "Python script failed");
//         }
//       });
//     } catch (err) {
//       reject(err);
//     }
//   });
// }

// export async function POST(req: NextRequest) {
//   try {
//     const body = await req.json();
//     const userId = body.userId;

//     if (!userId) {
//       return NextResponse.json({ error: "userId is required" }, { status: 400 });
//     }

//     const user = await prisma.user.findUnique({ where: { id: userId } });
//     if (!user) {
//       return NextResponse.json({ error: "User not found" }, { status: 404 });
//     }

//     const schemes = await prisma.scheme.findMany();

//     // Run Python with pre-filtered schemes based on state match
//     const eligibleSchemes = await runPython(user, schemes);

//     // Only send email if new eligible schemes exist
//     if (eligibleSchemes.length > 0 && user.email) {
//       const html = `
//         <h2>Hi ${user.name || "User"},</h2>
//         <p>Based on your profile, you are eligible for the following schemes:</p>
//         <ul>
//           ${eligibleSchemes
//             .map((s: any) => `<li><b>${s.name}</b> - ${s.details || "No description available"}</li>`)
//             .join("")}
//         </ul>
//         <p>Regards,<br/>Gov Scheme Portal</p>
//       `;
//       try {
//         await sendEmail(user.email, "Your Eligible Schemes", html);
//         console.log(`Eligibility email sent to ${user.email}`);
//       } catch (emailErr) {
//         console.error("Error sending eligibility email:", emailErr);
//       }
//     }

//     return NextResponse.json({
//       message: "Eligibility check complete. Recommendations have also been sent to email.",
//       eligibleSchemes,
//     });
//   } catch (err: any) {
//     console.error("Eligibility API Error:", err);
//     return NextResponse.json(
//       { error: "Eligibility check failed", details: err.toString() },
//       { status: 500 }
//     );
//   }
// }

// src/app/api/check-eligibility/route.ts
import { NextRequest, NextResponse } from "next/server";
import { PrismaClient } from "@prisma/client";
import { sendEmail } from "@/utils/actions/notifications/notifications";
import { auth } from "@/utils/actions/auth/auth";

const prisma = new PrismaClient();

export async function GET(req: NextRequest) {
  try {
    // 1️⃣ Get logged-in user
    const session = await auth();
    if (!session?.user?.id) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    const userId = session.user.id;
    const user = await prisma.user.findUnique({ where: { id: userId } });

    if (!user) {
      return NextResponse.json({ error: "User not found" }, { status: 404 });
    }

    // 2️⃣ Fetch all schemes
    const schemes = await prisma.scheme.findMany();

    // Filter by state or ministry
    const userState = user.state?.toLowerCase().trim();
    const stateFiltered = schemes.filter((scheme) => {
      if (!userState) return false;

      // Match state
      if (scheme.state && scheme.state.toLowerCase() === userState) {
        return true;
      }

      // If state in scheme starts with "ministry" → include
      if (scheme.state && scheme.state.toLowerCase().startsWith("ministry")) {
        return true;
      }

      return false;
    });

    // 3️⃣ Match eligibility
    const eligibleSchemeIds: string[] = [];

    for (const scheme of stateFiltered) {
      const criteriaList = Array.isArray(scheme.eligible)
        ? (scheme.eligible as any[])
        : [];

      let isEligible = true;

      for (const criteria of criteriaList) {
        // Education
        if (
          criteria.education &&
          user.education?.toLowerCase() !== criteria.education.toLowerCase()
        ) {
          isEligible = false;
          break;
        }

        // Occupation
        if (criteria.occupation) {
          if (Array.isArray(criteria.occupation)) {
            if (
              !criteria.occupation.some(
                (occ: string) =>
                  occ.toLowerCase() === user.occupation?.toLowerCase()
              )
            ) {
              isEligible = false;
              break;
            }
          } else {
            if (
              user.occupation?.toLowerCase() !==
              criteria.occupation.toLowerCase()
            ) {
              isEligible = false;
              break;
            }
          }
        }

        // Gender
        if (
          criteria.gender &&
          user.gender?.toLowerCase() !== criteria.gender.toLowerCase()
        ) {
          isEligible = false;
          break;
        }

        // Age
        if (criteria.minAge && user.age && user.age < criteria.minAge) {
          isEligible = false;
          break;
        }
        if (criteria.maxAge && user.age && user.age > criteria.maxAge) {
          isEligible = false;
          break;
        }

        // Caste Category ✅
        if (
          criteria.casteCategory &&
          user.castecategory?.toLowerCase() !==
            criteria.casteCategory.toLowerCase()
        ) {
          isEligible = false;
          break;
        }

        // Scheme Tags vs User Occupation ✅
        if (scheme.tags && scheme.tags.length > 0 && user.occupation) {
          const tagsMatch = scheme.tags.some(
            (tag: string) => tag.toLowerCase() === user.occupation?.toLowerCase()
          );
          if (!tagsMatch) {
            isEligible = false;
            break;
          }
        }
      }

      if (isEligible) {
        eligibleSchemeIds.push(scheme.id);
      }
    }

    // 4️⃣ Update user with eligible schemes
    await prisma.user.update({
      where: { id: userId },
      data: { eligibleSchemes: eligibleSchemeIds },
    });

    // Fetch those schemes
    const eligibleSchemes = await prisma.scheme.findMany({
      where: { id: { in: eligibleSchemeIds } },
    });

    // (Optional) send mail
    if (eligibleSchemes.length > 0 && user.email) {
      const html = `
        <h2>Hi ${user.name || "User"},</h2>
        <p>Based on your profile, you are eligible for the following schemes:</p>
        <ul>
          ${eligibleSchemes
            .map(
              (s) =>
                `<li><b>${s.name}</b> - ${
                  s.details || "No description available"
                }</li>`
            )
            .join("")}
        </ul>
      `;
      try {
        await sendEmail(user.email, "Your Eligible Schemes", html);
      } catch (err) {
        console.error("Email send failed:", err);
      }
    }

    return NextResponse.json({ eligibleSchemes });
  } catch (err) {
    console.error("Eligibility check error:", err);
    return NextResponse.json(
      { error: "Internal server error", details: err },
      { status: 500 }
    );
  }
}
